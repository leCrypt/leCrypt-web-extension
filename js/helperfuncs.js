...
function passItemGive(website, username, password) {
  var passItem =
    ...
    '     <input readonly class="website" style="margin-top: 4%; margin-bottom: 2%; margin-left: 1%; width: 85%; font-weight: bold; font-size: 16px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; vertical-align: middle;  border-style: solid; border-radius: 4px; border-color: black; border-width: 2px; font-size: large; box-shadow: 0px 0px;" value="' +
    website +
    '">' +
    ...
    '   <div style="width:80%; border: 2px solid #6c63ff; border-radius: 50px;"></div>' +
    '   <div style="display: flex; justify-content: space-between;">' +
    '   	<input readonly class="username" style="width: 85%; font-size: large; margin-left: 1%; margin-top: 2.5%; display: inline-block; word-break: break-word; vertical-align: middle; border-style: solid; border-radius: 4px; border-color: black; border-width: 2px; font-size: large; box-shadow: 0px 0px;" value="' +
    username +
    '">' +
    ...
    "   <div>" +
    '     <input readonly class="form-control password-box" type="password" style="vertical-align: middle; width:73%; margin: 0px; margin-right: 2%; border-style: solid; border-radius: 4px; border-color: black; border-width: 2px; font-size: large; box-shadow: 0px 0px;" value="' +
    password +
    '">' +
...
function passLoginsToDataPoint(data) {
  $.post(
    "http://localhost:8692/api/update/passes",
    JSON.stringify({ passes: data }),
    function (data) {
...
function passNotesToDataPoint(data) {
  $.post(
    "http://localhost:8692/api/update/notes",
...
function passHashToDataPoint() {
  chrome.storage.local.get(["secpassverify"], function (data) {
    if (data.secpassverify != undefined) {
      $.post(
        "http://localhost:8692/api/update/hash",
        JSON.stringify({ hash: data.secpassverify.sample }),
        function (data) {
