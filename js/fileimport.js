...
function receivedText() {
  if (service == "google") {
    ...
    importLogins(
      "type,name,notes,fields,login_uri,login_username,login_password",
      ",",
      6,
      7,
      8
...
function importLogins(
  ...
  if (lines[0].includes(verifier)) {
        ...
        if (i < lines.length && lines[i].includes(delimiter)) {
          ...
          var encryptedWebsite = CryptoJS.AES.encrypt(
            base_url(
              flag
                ? fields[urlIndex]
                : fields[urlIndex].slice(1, fields[urlIndex].length - 1)
            ),
            key
          ).toString();
    ...
    $("#loginImportSpan").text(
      "Please select a " + service + " passwords exported file (csv)!"
    );
  }
}
